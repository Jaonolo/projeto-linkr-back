...
export async function getPostByUser(req,res){
    ...
    try {
        ...
        const postsInfo = await db.query(`SELECT users."userName", users."profilePicture", 
                                                            posts.message, posts.link, COUNT(likes."postId") as likes
                                                            FROM users
                                                            JOIN posts ON posts."userId" = users.id
                                                            LEFT JOIN likes ON posts.id = likes."postId" 
                                                            WHERE users.id=$1
                                                            GROUP BY users."userName", users."profilePicture",
                                                            posts.message, posts.link`, [parseInt(req.params.id)]);
